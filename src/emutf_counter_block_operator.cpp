...
const mtf7::word_64bit *mtf7::emutf_counter_block_operator::unpack ( const mtf7::word_64bit *at_ptr ){
  ...
  if ( _16bit_word_a & 0x8000 )             *_error_status = mtf7::BLOCK_COUNTER_FORMAT;
  if ( (_16bit_word_b & 0x8000) != 0x8000 ) *_error_status = mtf7::BLOCK_COUNTER_FORMAT;
  if ( _16bit_word_c & 0x8000 )             *_error_status = mtf7::BLOCK_COUNTER_FORMAT;
  if ( _16bit_word_d & 0x8000 )             *_error_status = mtf7::BLOCK_COUNTER_FORMAT;
  if (*_error_status != mtf7::NO_ERROR) return 0;
